...
import { LOCK_TTL_MS, type IdempotencyRecord, type OwedSpend, type StorePort } from "./store.js";
  ...
  return {
      ...
      const rows = await sql.query(
        `UPDATE fleet_owed_spend SET leased_until = $1
         WHERE id IN (
           ...
           WHERE queued_tx IS NULL AND leased_until < $2
    ...
    async owedFor(depositor): Promise<Uint> {
      const [row] = await sql.query(
        "SELECT COALESCE(SUM(amount::numeric), 0)::text AS owed FROM fleet_owed_spend WHERE depositor = $1 AND queued_tx IS NULL",
