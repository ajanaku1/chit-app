...
import { sealKey, type FleetRecordLike } from "./bot-wallets.js";
...
export class FleetDriver {
  ...
  /** The challenge flow: hash the body, ask for a challenge, sign it, send the action with the envelope. */
  async signed(action: string, body: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const key = IDEMPOTENT.has(action) ? `fleet-bot-${action}-${this.#now().getTime().toString(36)}${Math.random().toString(36).slice(2, 10)}`.slice(0, 64) : undefined;
  ...
  /** Five fresh accounts and a commitment over their sealed keys; the service sees addresses and salts, never keys. */
  static generateAccounts(count: number, keySecret: string): FleetRecord["accounts"] {
    return Array.from({ length: count }, () => {
      ...
      return {
        ...
        sealedKey: sealKey(privateKey, keySecret),
  ...
  async createFleet(input: { accounts: FleetRecord["accounts"]; chainId: number; router: Address; fn: string; maxTradeValue: bigint; perAccountGas: bigint; days: number }): Promise<FleetRecord> {
    ...
    const confirmed = await this.signed("confirmRecovery", { campaign, vaultConfirmed: true });
  ...
  async activate(campaign: string, drawWei: bigint): Promise<{ state: string; accounts: Address[] }> {
    const r = await this.signed("activate", { campaign, draw: drawWei.toString() });
    return { state: String(r["state"] ?? ""), accounts: ((r["accounts"] as Address[] | undefined) ?? []) };
  }

  async status(campaign: string): Promise<Record<string, unknown>> {
  ...
  async buy(campaign: string, accounts: Address[], token: Address, valueWei: bigint): Promise<Record<string, unknown>> {
    return this.signed("buy", { campaign, accounts, token, value: valueWei.toString() });
  }

  async control(action: "pause" | "resume" | "close", campaign: string): Promise<Record<string, unknown>> {
    return this.signed(action, { campaign });
  }
...
/** Where a fleet is in its life, in one word a card can colour. */
export const fleetPhase = (record: FleetRecord | undefined): "none" | "created" | "activating" | "active" | "paused" | "closed" => {
  if (!record) return "none";
  const s = record.state.toLowerCase();
  if (s.includes("closed")) return "closed";
  if (s.includes("paused")) return "paused";
  if (s.includes("activating") || s.includes("funding")) return "activating";
