 ...
 * the bot is refused; after the owner's revoke it is refused for good; the
 * owner takes the tokens and the ETH back. Chit is nowhere in it.
 */
describe("Session keys on Uniswap v4 (46630 fork)", () => {
  it("a bot trades through a bounded key with a kill switch", async () => {
    ...
    await owner!.writeContract({ address: account, abi: SESSION_ACCOUNT_ABI, functionName: "withdrawToken", args: [token.address, owner!.account.address, bought] });
    await owner!.writeContract({ address: account, abi: SESSION_ACCOUNT_ABI, functionName: "withdraw", args: [owner!.account.address, parseEther("0.01") - amount] });
    assert.equal(await token.read.balanceOf([owner!.account.address]), bought);
    assert.equal(await publicClient.getBalance({ address: account }), 0n);
    console.log(`session key bought ${bought} FLEET for ${amount} wei through the live router, then was revoked; the owner holds the tokens`);
  });
});
